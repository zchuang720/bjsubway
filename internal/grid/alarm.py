...
import utils
import properties
import  internal.grid.data as grid_data
from internal.grid.process import *


def grid_alarm(pred_result, context, **kwargs):
    ret = deepcopy(grid_data.grid_alarm_result)
    refine_result = deepcopy(pred_result)
    logger = logging.getLogger(context['logger_name'])

    # 修改标签类名(en => zh)
    refine_result.names = grid_data.translated_cls_name
    ...
    # 检测警报事件
    if check_steel_state(pred_result):
        alarm_event_id.append(18)   # 18.暗挖未及时喷射混凝土
    if check_woking_face_state(pred_result):
